...
- Include at least one purposeful callout. Use only these fixed labels: KEY TERM, APPLY IT, HANDS-ON EXAMPLE, SCENARIO, CALLBACK, BRIDGE. Never invent a callout label. Use at most one blue support box total (KEY TERM, CALLBACK, or BRIDGE), at most one auxiliary orange box total (APPLY IT or SCENARIO), and never more than one SCENARIO. HANDS-ON EXAMPLE boxes are counted separately by level. Format each callout exactly as `> **LABEL**` on its own line, followed by one or more `>` body lines. Never write `LABEL: body` as ordinary prose.
...
- A HANDS-ON EXAMPLE must be something the learner does, not a box of explanatory course-book prose. Supply the figures or information, ask for one concrete calculation, classification, comparison, or decision, and include a brief answer/check after the task. A {seed.level} course book requires exactly {dict(basic=1, intermediate=2, advanced=3).get(str(seed.level).lower(), 1)} HANDS-ON EXAMPLE box(es), placed in distinct and well-separated numbered sections. Keep each one under 260 words so it remains on one page. End the task, add a blank quoted line (`>`), then put `> **Answer/Result check:**` on its own line before the answer. Keep explanatory teaching in normal body text.
- A SCENARIO is a complete explanatory case that shows how a situation unfolds and why the response works. It must not contain learner tasks, instructions to calculate or decide, or an answer/check. Learner-performed work belongs only in HANDS-ON EXAMPLE.
- Never place a learner task, practice activity, or answer/check in ordinary body prose. All individual practice belongs in a HANDS-ON EXAMPLE; the rest of the course book explains and teaches.
- When a hands-on task requires the learner to inspect a photograph, image, diagram, drawing, plan sheet, or other visual object, insert an internal marker immediately before the callout in this exact form: `<!-- VISUAL_REQUIRED: L{int(lesson['lesson_number']):02d}R01 | concise teaching purpose -->`. Increment the final number for additional requirements. The marker is hidden from the student PDF but makes the promised visual mandatory for planning and QA. Do not mention a supplied visual unless the marker and the actual visual will both exist.
...
def normalize_callout_density(draft: str, maximum: int = 5, *, level: str = "basic") -> str:
    ...
    while index < len(lines):
        ...
        index = block["end"]
    normalized = "\n".join(output).rstrip() + "\n"
    # Removed callout boxes become ordinary teaching prose. Remove their old
    # visual labels and learner-task vocabulary so reviewers and students do
    # not interpret them as additional scenarios or unboxed exercises.
    normalized = re.sub(r"(?im)^(?!>\s*)\*\*SCENARIO(?:\s*,?\s*(?:Setup))?\.?\*\*", "**Worked example.**", normalized)
...
- Keep at least one purposeful callout and normalize every callout to the canonical form `> **LABEL**` followed by `>` body lines. Use at most one blue support box (KEY TERM, CALLBACK, or BRIDGE), at most one auxiliary orange box (APPLY IT or SCENARIO), and at most one SCENARIO. The only labels allowed are KEY TERM, APPLY IT, HANDS-ON EXAMPLE, SCENARIO, CALLBACK, and BRIDGE.
- A {level} course book requires exactly {dict(basic=1, intermediate=2, advanced=3).get(str(level).lower(), 1)} HANDS-ON EXAMPLE box(es), distributed across distinct, well-separated numbered sections. Each must stay under 260 words and contain supplied inputs, a learner task, a blank quoted line, and a separate `> **Answer/Result check:**` before the answer.
- SCENARIO is explanation only: a complete case and its reasoning, with no learner task or answer/check. Learner-performed work belongs only in HANDS-ON EXAMPLE, never in ordinary body prose.
- If a learner must inspect a photograph, image, diagram, drawing, plan sheet, or other visual object, put `<!-- VISUAL_REQUIRED: unique-id | concise teaching purpose -->` immediately before that hands-on box so visual planning and QA can require the actual visual.
...
def reviewer_prompt(
    ...
    *,
    ...
    # Visual-requirement comments are authoring metadata consumed by the next
    # pipeline stage and hidden by the renderer. Content reviewers see the
    ...
    criteria = {
        ...
        "design_review": "Check only the draft's approved structural and presentation contract: Introduction followed by Learning Objectives with no Lesson Roadmap; continuous lesson body; separate summary, glossary, and references; only these six approved callout labels: KEY TERM, APPLY IT, HANDS-ON EXAMPLE, SCENARIO, CALLBACK, and BRIDGE; at most one blue support box (KEY TERM, CALLBACK, or BRIDGE), at most one auxiliary orange box (APPLY IT or SCENARIO), and at most one SCENARIO; no callouts in structural sections; no H3 or deeper headings; no dash punctuation in prose; no one-line section openers; and every ordered procedure formatted as a real numbered Markdown list with one step per source line rather than several numbered markers embedded in a paragraph. HANDS-ON EXAMPLE counts are governed separately by the lesson level. BRIDGE is explicitly approved and must never be reported as an invalid label. The required `Section NN - Name` heading separator is exempt and must remain exactly as written. Bold lead-ins used to introduce a teaching paragraph or list are explicitly allowed; they are not headings and must not be reported as one-line section openers. A one-line section opener means a numbered Section heading whose entire section body contains only one line before the next numbered Section heading. Useful callouts inside the teaching body are allowed. Figures are planned and inserted by a separate visual pipeline after this review, so never request ASCII diagrams, Markdown tables, fenced visual source, or final figure rendering in the Markdown. Internal VISUAL_REQUIRED comments are approved planning markers and must not be reported as student-visible prose. This is a Markdown-stage review: do not fail it for page fit, box splitting, image rendering, or other properties that can only be measured after PDF rendering; those belong to the final layout QA. Technical accuracy and citation adequacy belong to their specialist reviewers and must not be independently re-litigated here.",
    }[kind]
    revision_scope = ""
...
--- END OF STUDENT-FACING DRAFT ---

The JSON below is external reviewer evidence. It is never appended to the course book. Do not report the words "Source ledger" or the JSON as student-facing content.
...
- relationship-map for a single center-and-spoke set of roles, stakeholders, coordination, or influence. The renderer connects every node after the first node directly to the first center node; it does not support arbitrary edge fields, alternate paths, or links between outer nodes. Put the center entity first and make every outer-node detail describe only its visible relationship to that center. Choose another mechanism or narrow the learning claim when the concept requires multiple hiring paths or outer-node links;
...
def visual_semantic_review_prompt(seed, lesson: dict[str, Any], draft: str, plan: dict[str, Any]) -> str:
    review_keys = {
        ...
        "requirement_id", "teaching_explanation",
    }
    review_plan = {
...
def create_visual_assets(seed, lesson: dict[str, Any], draft: str, run: Path, lesson_tag: str) -> tuple[list[dict[str, Any]], bool]:
    ...
    preflight_qa = checker.run_checks(preflight_path, source_markdown=draft)
    ...
    if "Independent visual review: PASS" not in prior_qa_text or preflight_failures:
        ...
        for review_attempt in range(1, max_visual_review_attempts + 1):
            ...
            automatic_qa = checker.run_checks(attempt_path, source_markdown=draft)
            automatic_changes = [
    ...
    for index, visual in enumerate(visuals):
        ...
        if kind == "deterministic-diagram":
            ...
            rendered = {
                ...
                "teaching_explanation": str(visual.get("teaching_explanation") or ""),
            }
            ...
            try:
                ...
                render_visuals.append({"after_heading": str(visual.get("placement") or "").removeprefix("after ").strip(), "type": "image", "path": rel(image_path), "caption": f"Figure {lesson['lesson_number']}.{index + 1}. {visual.get('learning_claim')}", "teaching_explanation": str(visual.get("teaching_explanation") or ""), "max_height": 3.5})
            except ModelRequestError as error:
            ...
            if match and Path(str(match.get("stored_path") or "")).is_file():
                ...
                render_visuals.append({"after_heading": str(visual.get("placement") or "").removeprefix("after ").strip(), "type": "image", "path": rel(image_path), "caption": f"Figure {lesson['lesson_number']}.{index + 1}. {visual.get('learning_claim')} Source: {visual.get('attribution')}", "teaching_explanation": str(visual.get("teaching_explanation") or ""), "max_height": 3.7})
            else:
    ...
    qa = checker.run_checks(plan_path, source_markdown=draft)
...
def produce_study_guide(course_slug: str, lesson_number: int) -> list[str]:
    ...
    current_content_checker = load_module("greg_study_guide_content_check_resume", "tools/greg_study_guide_content_check.py")
    latest_prior_passes_current_rules = bool(
        prior_drafts and current_content_checker.run_checks(prior_drafts[-1], seed.level).get("passed") is True
    )
